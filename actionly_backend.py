...
ROOM_TYPES = {
    ...
    'family': {
        ...
        'keywords': ['family', 'children', 'kids', 'large', 'connecting'],
        'guests': 4
    },
    'junior_suite': {
        'name': 'Junior Suite',
    ...
    'suite': {
        ...
        'description': 'Luxury accommodation with separate living space',
        'keywords': ['suite', 'apartment', 'luxury', 'executive', 'presidential'],
        'guests': 3
    }
}
...
def search_hotels_booking_api(location_id, checkin, checkout, adults, rooms, room_type=None):
    """Search hotels using Booking.com API with room type"""
    ...
    # Add room type specific parameters if provided
    if room_type and room_type in ROOM_TYPES:
        room_info = ROOM_TYPES[room_type]
        querystring["adults"] = room_info['guests']
        
        # Add children for family rooms
        if room_type == 'family':
            querystring["children"] = 2
...
def create_booking_url(hotel, city_info, checkin, checkout, adults, rooms, city_key, room_type=None):
    ...
    if hotel_id and hotel_name:
        ...
        # Add room type parameters if specified
        if room_type and room_type in ROOM_TYPES:
            room_info = ROOM_TYPES[room_type]
            base_params['room_type'] = room_type
            base_params['group_adults'] = room_info['guests']
...
def analyze_hotel_for_room_type(hotel, room_type):
    ...
    # Default: include all hotels (API filtering is primary)
    return True
...
def process_hotel_data(hotels_data, city_info, checkin, checkout, adults, rooms, city_key, room_type=None):
    ...
    for i, hotel in enumerate(hotels_data):
        # Check if hotel matches room type (optional filtering)
        if not analyze_hotel_for_room_type(hotel, room_type):
            continue
        ...
        processed_hotel = {
            ...
            'room_type_match': room_type if room_type else 'standard'
        }
...
def home():
    ...
    <head>
        ...
        <style>
            ...
            .room-type { background: #fff3e0; padding: 10px; border-radius: 4px; }
        </style>
    </head>
    <body>
        ...
        <div class="feature">
            ...
            Search hotels by room type: Single, Double, Family, Junior Suite, Suite/Apartment
        </div>
        
        <div class="feature">
            <strong>✅ ENHANCED: Hotel Name-Based Booking URLs</strong><br>
            CTA buttons now use hotel names for better targeting and SEO
        </div>
        ...
        <div class="endpoint">
            ...
            <em>Now with room type filtering and enhanced URLs</em>
        </div>
        <div class="endpoint">
            <strong>/api/cities</strong> - List all 29 cities
        </div>
        <div class="endpoint">
            <strong>/api/room-types</strong> - List all room types with descriptions
        </div>
        <div class="endpoint">
            <strong>/test</strong> - Test Stockholm hotels with Junior Suite
        </div>
        ...
        <div class="room-types">
            ...
            <div class="room-type">
                ...
                <small>{{ room_info.guests }} guest(s)</small>
            </div>
            {% endfor %}
        </div>
...
def get_hotels():
    ...
    if room_type and room_type not in ROOM_TYPES:
        return jsonify({'error': f'Room type {room_type} not supported. Available: {list(ROOM_TYPES.keys())}'}), 400
    ...
    processed_hotels = process_hotel_data(
        ...
        room_type  # Pass room type for filtering
    )
    
    # Get room type info for response
...
@app.route('/test')
def test_stockholm():
    """Test endpoint with Stockholm Junior Suite hotels"""
    return get_hotels()
...
if __name__ == '__main__':
    ...
    print("🌍 Enhanced: Hotel name-based booking URLs with localization")
    print("🔗 Frontend will connect to: http://localhost:5000")
    print("📋 Test API: http://localhost:5000/test")
    print("✅ Complete room type integration")

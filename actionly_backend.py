# STAYFINDR BACKEND v12.6 - Final Dynamic Version
# FINAL FIX: Implemented dynamic Geo ID lookup for the legacy TripAdvisor API (tripadvisor16).
# This removes the reliance on outdated static IDs from the CSV file, which was the root cause of empty results for certain cities.
...
def process_tripadvisor_hotels(api_data):
    ...
    for hotel in hotels_data[:TRIPADVISOR_HOTEL_LIMIT]:
        ...
        processed.append({
            ...
            'source': 'tripadvisor',
            'booking_url': f"https://www.tripadvisor.com{hotel.get('commerceInfo', {}).get('externalUrl')}" if hotel.get('commerceInfo', {}).get('externalUrl') else f"https://www.tripadvisor.com/Search?q={quote_plus(hotel.get('title', 'Hotel'))}"
        })
    return processed
...
# --- Flask Routes ---
@app.route('/')
def home(): return render_template_string('<h1>STAYFINDR Backend v12.6</h1><p>Final and stable version.</p>')
...
@app.route('/test')
def test_endpoint_route(): return jsonify({'status': 'STAYFINDR Backend v12.6 Active'})

# --- Application Startup ---
if __name__ == '__main__':
    logging.info("🚀 Starting STAYFINDR Backend v12.6...")
